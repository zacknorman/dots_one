...
var AppStateWatcher = /** @class */ (function (_super) {
    ...
    Object.defineProperty(AppStateWatcher.prototype, "canShowRememBear", {
        get: function () {
            return this.internalValue.canShowRememBear;
        },
        set: function (canShowRememBear) {
            this.internalValue.canShowRememBear = canShowRememBear;
            this.triggerWatch();
        },
        enumerable: true,
        configurable: true
...
function getPopupState() {
    ...
    return {
        ...
        canShowRememBear: appState.canShowRememBear,
...
var TB_REF = "client_" + utils["e" /* getBrowserTitle */]().toLowerCase();
var RB_BASE_URL = "https://www.remembear.com";
function outOfData() {
    var tbRef = browser.tabs.create({ url: BASE_URL + "/account/upgrade?ref_id=" + TB_REF + "&notice=no_data&v=" + utils["p" /* versionString */] });
}
function lowAccount(emailConfirmed) {
    if (emailConfirmed === true) {
        browser.tabs.create({ url: BASE_URL + "/account/upgrade?ref_id=" + TB_REF + "&notice=low_data&v=" + utils["p" /* versionString */] });
    }
    else {
        browser.tabs.create({ url: BASE_URL + "/account/upgrade?ref_id=" + TB_REF + "&v=" + utils["p" /* versionString */] });
    }
}
...
function openRememBear() {
    browser.tabs.create({ url: RB_BASE_URL + "/try?utm_source=tunnelbear-extension" });
}
...
var token_manager_TokenManager = /** @class */ (function () {
    ...
    TokenManager.prototype.getToken = function (proxy) {
        ...
        token_manager_log(logger["a" /* Level */].DEBUG, "[Get-Token] - current token for " + proxy + " " + proxyToken);
    ...
    TokenManager.prototype.newToken = function (proxy) {
        ...
        if (this.requestInProgress === false || this.registerPromise === undefined) {
            token_manager_log(logger["a" /* Level */].INFO, "Fetching new token");
    ...
    TokenManager.prototype.validateToken = function (proxy, proxyToken) {
        token_manager_log(logger["a" /* Level */].DEBUG, "Validating token");
    ...
    TokenManager.prototype.setToken = function (proxy, token) {
        token_manager_log(logger["a" /* Level */].DEBUG, "Setting token: " + token + " for proxy: " + proxy);
...
var browser_FirefoxBrowser = /** @class */ (function () {
    ...
    FirefoxBrowser.prototype.isRememBearInstalled = function () {
        //TODO: it seems like management permissions were removed on purpose for FF?
...
var browser_ChromeBrowser = /** @class */ (function () {
    ...
    ChromeBrowser.prototype.browserSetProxy = function (isEnabled, servers) {
        browser_log(logger["a" /* Level */].DEBUG, "---- setting browser proxy ---");
    ...
    ChromeBrowser.prototype.browserRegisterOnAuthListener = function () {
        browser_log(logger["a" /* Level */].DEBUG, "---- registering for onAuthRequired ---");
        chrome.webRequest.onAuthRequired.addListener(function (details, callback) {
            browser_log(logger["a" /* Level */].DEBUG, "---- onAuthRequired ---");
            var proxy = details.challenger.host;
            var tabId = details.tabId;
            browser_log(logger["a" /* Level */].INFO, "onAuthRequired for proxy");
    ...
    ChromeBrowser.prototype.isRememBearInstalled = function () {
        return new Promise(function (resolve) {
            return chrome.management.getAll(function (apps) {
                resolve(apps.some(function (app) { return app.name === "RememBear"; }));
            });
        });
    };
...
function toggleIcon(isToggled) {
    if (isToggled) {
        browser.browserAction.setIcon({
            path: {
                19: __webpack_require__(84),
                38: __webpack_require__(83)
            }
        });
    }
    else {
...
function browser_isRememBearInstalled() {
    return whichBrowser.isRememBearInstalled();
}
...
function startProxyTimer() {
    proxyTimer = window.setInterval(proxyTimerCallback, 2000);
}
...
function getRememBear() {
    appState.canShowRememBear = false;
    utils["o" /* storageSet */]("get_remembear", Date.now());
    closeRememBear();
}
function closeRememBear() {
    appState.canShowRememBear = false;
    ports["c" /* sendMessage */](ports["a" /* PortName */].POPUP_STATE, getPopupState());
    utils["o" /* storageSet */]("remembear", Date.now());
}
...
Promise.all([
    utils["m" /* storageGet */](["lastCountry", "twitter", "get_remembear", "remembear"]),
    browser_isRememBearInstalled()
]).then(function (_a) {
    var storage = _a[0], isRememBearInstalled = _a[1];
    // lastCountry
    var country = storage.lastCountry;
    var toggled = appState.toggled;
    ...
    // twitter
    var twitterLastTime = storage.twitter;
    //The default value for `State.appState.canShowTwitter` is `true`
    if (regResponse.fullVersion) {
        appState.canShowTwitter = false;
    }
    else if (twitterLastTime !== undefined) {
        appState.canShowTwitter = utils["c" /* dateOlderThan */](twitterLastTime, 7);
    }
    //RememBear prompt  
    var rememBearDismissDate = storage.remembear;
    //Even though it's not here (in the vue itself for not good reasons) this is for now only shown to free users
    var remembearPromptConditions = [
        isRememBearInstalled === false,
        (!appState.canShowTwitter || (appState.canShowTwitter === true && Math.floor(Math.random() * 10) % 3 === 1)),
        twitterLastTime === undefined || utils["c" /* dateOlderThan */](twitterLastTime, 1),
        rememBearDismissDate === undefined
            || (utils["c" /* dateOlderThan */](rememBearDismissDate, 30) && !regResponse.fullVersion) //for free user, last prompt at 30 days
            || (utils["c" /* dateOlderThan */](rememBearDismissDate, 60) && regResponse.fullVersion) //for paid user, last prompt at 60 days
    ];
    //Any condition that's false will cause canShowRememBear to be false
    appState.canShowRememBear = !remembearPromptConditions.some(function (condition) { return condition === false; });
    //When RB prompt is supposed to show, set the canShowTwitter to false or else it takes precedence
    if (appState.canShowRememBear === true) {
        appState.canShowTwitter = false;
    }
    core_log(logger["a" /* Level */].DEBUG, "canShowTwitter options: " + appState.canShowTwitter);
    core_log(logger["a" /* Level */].DEBUG, "canShowRememBear options: " + remembearPromptConditions);
    core_log(logger["a" /* Level */].DEBUG, "Setting canShowRememBear: " + appState.canShowRememBear);
}).then(function () {
